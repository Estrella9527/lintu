...
export const RELEASES: ReleaseEntry[] = [
  {
    version: '0.3.8',
    date: '2026-07-24',
    highlights: '上传链路简化为“确认即入图库”，压缩完成后自动同步 OSS，打标不再阻塞发布',
    sections: [
      {
        kind: 'improved',
        items: [
          '**直接上传图库**:选择图片后可逐张确认或移除；确认即保留本地原图、生成压缩发布版，并在完成后自动同步 OSS',
          '**草稿明确发布**:画布草稿和历史本地图片支持多选“上传到图库”，需明确确认才会进入发布链路，避免误上传',
          '**UGC 独立上架**:文件同步 OSS 后默认仍不上架；是否进入 UGC 匹配由“上架”单独决定',
        ],
      },
      {
        kind: 'fixed',
        items: [
          '**上传后无去向**:批量压缩不再停留在本地或依赖人工审核；压缩完成会自动继续 OSS 同步',
          '**打标阻塞发布**:标签改为检索与匹配增强，不再因为未打满必填维度阻止图库上传',
          '**审核流程冗余**:新上传、画布发布和 OSS 回填不再进入审核队列；旧审核数据仅保留兼容查看',
        ],
      },
    ],
  },
