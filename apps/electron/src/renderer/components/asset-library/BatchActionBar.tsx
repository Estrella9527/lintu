...
import {
  CheckCircle, ChevronDown, Copy, Download, FolderInput, FolderPlus,
  Loader2, RotateCw, Rocket, ShieldCheck, Tag as TagIcon,
...
export function BatchActionBar({ selectedCount, selectedIds, onClear, mode = 'library' }: BatchActionBarProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 px-3 py-1.5 mb-2 rounded-lg bg-accent/5 border border-accent/20">
      ...
      <PublishToLibraryButton ids={ids} onDone={onClear} />
...
/** 历史「仅本地」图片和画布草稿的明确发布入口。 */
function PublishToLibraryButton({ ids, onDone }: { ids: string[]; onDone: () => void }) {
  const queryClient = useQueryClient()
  const publish = useMutation({
    mutationFn: () => api.images.publishToLibrary(ids),
    onSuccess: (result) => {
      toast.success(
        result.published > 0
          ? `已把 ${result.published} 张上传到图库，压缩完成后将自动同步 OSS`
          : '所选图片已在图库，已重新检查压缩与 OSS 同步状态',
      )
      onDone()
      queryClient.invalidateQueries({ queryKey: ['images'] })
      queryClient.invalidateQueries({ queryKey: ['image-folders'] })
    ...
    onError: (error: Error) => toast.error(error.message || '上传到图库失败'),
  })
  ...
  return (
    <Button
      ...
      title="明确发布到图库：保留原图，生成压缩发布版，并在完成后同步 OSS"
      onClick={() => {
        if (!window.confirm(
          `确认把选中的 ${ids.length} 张图片上传到图库吗？\n\n系统会保留原图，生成压缩发布版，并在完成后同步 OSS。`,
        )) return
        publish.mutate()
      }}
    >
      {publish.isPending ? <Loader2 size={12} className="mr-1 animate-spin" /> : <Upload size={12} className="mr-1" />}
      {publish.isPending ? '上传中…' : '上传到图库'}
    </Button>
  )
}
